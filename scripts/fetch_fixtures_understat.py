...
async def fetch_league(league: str, horizon_days: int, season_override: int | None = None) -> List[Dict[str, str]]:
...
def load_understat_fixtures(league: str, days: int = 14, season: int | None = None) -> pd.DataFrame:
    ...
    rows = asyncio.run(fetch_league(league_norm, days, season_override=season))
    if not rows:
