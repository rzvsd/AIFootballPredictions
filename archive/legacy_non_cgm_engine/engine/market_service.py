...
def build_market_book(predictions: List[Dict[str, Any]], cfg: Dict[str, Any]) -> pd.DataFrame:
    ...
    debug_prob = bool(cfg.get("debug_prob") or str(os.getenv("BOT_DEBUG_PROB", "")).lower() in {"1", "true", "yes", "on"})
    ...
    for pred in predictions:
        ...
        if P is None:
            continue
        markets_raw = _evaluate_all_markets(P, ou_lines=ou_lines, intervals=intervals)
        markets = copy.deepcopy(markets_raw)
        ...
        if debug_prob:
            raw_ou25 = (markets_raw.get("OU") or {}).get("2.5") or {}
            cal_ou25 = (markets.get("OU") or {}).get("2.5") or {}
            debug_rows.append(
                {
                    "date": date_val,
                    "home": home,
                    ...
                    "mu_h": pred.get("mu_h"),
                    "mu_a": pred.get("mu_a"),
                    "raw_1x2_H": markets_raw["1X2"].get("H"),
                    "raw_1x2_D": markets_raw["1X2"].get("D"),
                    "raw_1x2_A": markets_raw["1X2"].get("A"),
                    "cal_1x2_H": markets["1X2"].get("H"),
                    "cal_1x2_D": markets["1X2"].get("D"),
                    "cal_1x2_A": markets["1X2"].get("A"),
                    "raw_ou25_over": raw_ou25.get("Over"),
                    "raw_ou25_under": raw_ou25.get("Under"),
                    "cal_ou25_over": cal_ou25.get("Over"),
                    "cal_ou25_under": cal_ou25.get("Under"),
                }
            )

    df = pd.DataFrame(rows)
    if debug_prob and debug_rows:
        try:
            rep = Path("reports"); rep.mkdir(parents=True, exist_ok=True)
            out_path = rep / f"{league}_prob_debug.csv"
            pd.DataFrame(debug_rows).to_csv(out_path, index=False)
            print(f"[debug] Probability breakdown saved -> {out_path}")
        except Exception:
            pass
    return df
