...
def _fill_odds_for_df(df: pd.DataFrame, league: str, with_odds: bool = True) -> pd.DataFrame:
    """
    Attach real odds to the DataFrame; leave missing values as NaN so downstream
    value_service can synthesize and correctly flag synthetic prices.
    """
    return odds_service.fill_odds_for_df(df, league, with_odds=with_odds)
...
def main() -> None:
    ...
    if cfg.get('log_bets', True):
        ...
        for _, r in show.iterrows():
            ev = float(r['EV'])
            # Skip rows without a finite EV (e.g., synthetic odds) to avoid staking on placeholders.
